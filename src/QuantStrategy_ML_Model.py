...
def load_data_from_csv(file) -> pd.DataFrame:
    ...
    try:
        ...
        if datetime_col is None:
            raise ValueError(
                "CSV must have a datetime column (datetime_ist, atetime_ist, datetime, or date)"
            )
...
def create_labels(
    ...
    if method == "threshold":
        ...
        d["direction"] = np.where(
            d["future_ret_pct"] > terciles[0.67],
            1,
        ...
        d["direction"] = np.where(
            d["future_ret_pct"] > quintiles[0.8],
            1,
...
def generate_signals_advanced(
    ...
    for i in range(1, len(d)):
        ...
        long_trend = (
            row["close"] > row["ema20"] > row["ema50"] and row["close"] > row["vwap"]
        )
        short_trend = (
            row["close"] < row["ema20"] < row["ema50"] and row["close"] < row["vwap"]
        )
        ...
        if not in_trade:
            ...
            if direction == 1:
                ...
                if high >= target_price:
                    ...
                    exit_reason = "SL"
                elif use_trailing_stop and low <= trailing_stop:
                ...
                if low <= target_price:
                    ...
                    exit_price = stop_price
                    exit_reason = "SL"
                elif use_trailing_stop and high >= trailing_stop:
                    exit_price = trailing_stop
                    exit_reason = "TRAIL"
...
# ------------------------------------------
# 8. COMPREHENSIVE METRICS (ROBUST)
# ------------------------------------------
def compute_comprehensive_metrics(
    trades: pd.DataFrame, results: pd.DataFrame, initial_capital: float = 100000
):
    """Calculate institutional-grade performance metrics (safe even if no trades)."""

    # Equity-based data (always try to use it)
    equity_curve = (
        results["equity"].dropna()
        if results is not None and "equity" in results
    ...
    if trades is None or trades.empty:
        ...
        if not equity_returns.empty and equity_returns.std() > 0:
            volatility = equity_returns.std() * np.sqrt(252 * 26)
            sharpe = (
                equity_returns.mean() / equity_returns.std() * np.sqrt(252 * 26)
            )
            downside_returns = equity_returns[equity_returns < 0]
            downside_std = (
                downside_returns.std() * np.sqrt(252 * 26)
                if not downside_returns.empty
                else 0
            )
            sortino = (
                equity_returns.mean() / downside_std * np.sqrt(252 * 26)
                if downside_std > 0
                else 0
            )
    ...
    # If we have trades, compute full trade-based metrics
...
def plot_comprehensive_dashboard(
    results: pd.DataFrame, trades: pd.DataFrame, metrics: dict
):
    ...
    ax2.fill_between(
        drawdown.index, drawdown.values, 0, color="#E63946", alpha=0.6
    )
    ...
    if not trades.empty:
        ...
        ax4.set_title(
            f"Win Rate: {metrics['win_rate']:.1%}", fontsize=12, fontweight="bold"
        )
    ...
    if not trades.empty:
        ...
        ax5.fill_between(
            range(len(cum_pnl)), cum_pnl, 0, alpha=0.3, color="#F77F00"
        )
    ...
    if not trades.empty and hasattr(trades["exit_time"].iloc[0], "month"):
        ...
        monthly_returns = (
            trades_copy.groupby(["year", "month"])["pnl"]
            .sum()
...
"""
    ...
    plt.suptitle(
        "ML Trading Strategy - Comprehensive Performance Dashboard",
        fontsize=16,
        fontweight="bold",
        y=0.995,
    )
...
def plot_signal_analysis(results: pd.DataFrame):
    ...
    ax3 = axes[2]
    colors = [
        "green" if row["signal"] == 1 else "red" if row["signal"] == -1 else "gray"
...
def main():
    ...
    model_choice = st.sidebar.selectbox(
        "Select Model",
        ["GaussianNB", "LogisticRegression", "RandomForest", "GradientBoosting"],
    )
    var_smoothing = st.sidebar.number_input(
        "Var Smoothing (NB only)",
        min_value=1e-12,
        max_value=1e-6,
        value=1e-9,
        format="%.2e",
    ...
    label_method = st.sidebar.selectbox(
        "Label Method", ["threshold", "tercile", "quintile"]
    )
    return_threshold = st.sidebar.slider(
        "Return Threshold (%)", min_value=0.1, max_value=2.0, value=0.2, step=0.1
    )
    horizon = st.sidebar.slider(
        "Prediction Horizon (bars)", min_value=1, max_value=10, value=1
    )
    ...
    vol_threshold = st.sidebar.slider(
        "Volume Threshold", min_value=0.5, max_value=2.0, value=1.0, step=0.1
    )
    ...
    sl_mult = st.sidebar.slider(
        "Stop Loss (ATR Multiple)", min_value=0.5, max_value=3.0, value=1.0, step=0.1
    )
    tp_mult = st.sidebar.slider(
        "Take Profit (ATR Multiple)", min_value=1.0, max_value=5.0, value=1.5, step=0.1
    )
    ...
    trailing_mult = st.sidebar.slider(
    ...
    if uploaded_file is not None:
        try:
            ...
            st.success(
                f"✅ Data loaded: {len(df)} rows from {df.index[0]} to {df.index[-1]}"
            )
            ...
            if run_button:
                ...
                st.success(
                    f"✅ ML dataset built: {len(X)} samples, {len(feature_cols)} features"
                )
                ...
                with col2:
                    st.metric("Class 1 (Up) Samples", int(y.sum()))
                    st.metric("Class 0 (Down) Samples", int(len(y) - y.sum()))

                # ---- SAFETY CHECK: enough data & both classes ----
                unique_classes = np.unique(y)
                if len(X) < 30 or len(unique_classes) < 2:
                    st.error(
                        "❗ Not enough usable labeled samples for training.\n\n"
                        "- Need **at least ~30 samples**\n"
                        "- And **both classes (0 and 1)** after labeling\n\n"
                        "Try using more data or adjusting the labeling method / return threshold."
                    )
                    return
                ...
                st.success(
                    f"✅ Signals generated: {signal_counts.get(1, 0)} Longs, {signal_counts.get(-1, 0)} Shorts"
                )
                ...
                metrics = compute_comprehensive_metrics(
                    trades, results, initial_capital
                )
                ...
                with st.spinner("Creating visualizations..."):
                    fig_dashboard = plot_comprehensive_dashboard(
                        results, trades, metrics
                    )
                ...
                if not trades.empty:
                    ...
                    st.warning(
                        "No trades executed during backtest period (but equity metrics were still computed if possible)."
                    )
                ...
                col1, col2 = st.columns(2)
                with col1:
                    csv_results = results.to_csv().encode("utf-8")
                    st.download_button(
                ...
                with col2:
                    if not trades.empty:
                        csv_trades = trades.to_csv().encode("utf-8")
                        st.download_button(
                            "📥 Download Trades CSV",
                            csv_trades,
                            "trades_log.csv",
...
> This tool is for educational backtesting only. Past performance does **not** guarantee future results.
"""
        )
